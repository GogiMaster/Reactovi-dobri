...
const newPostObj:PostType = {
  ...
  hashtags: ["oblak", "priroda", "de lajkaj"],
  img_url:
...
useEffect(() => {
  ...
  return (
    ...
    <h1>Instagram</h1> 
    <img className="post__bigger" src={plus} onClick={()=> handleModal()}/>
    </div>
    <hr />
    {posts.map((post)=>{
      return<>
   ...
   <div className="post__tab">
    <h1>{post.title}</h1>
    <img className="post__svg" onClick={()=>deletePost(post.id)} src={trash}/>
...
<div className="post__modal">
  <input id="post_title" required className="post__modal__input" type="text" placeholder="Title" />
  <textarea id="post_desc" required name="description" className="post__modal__input" placeholder="Description"></textarea>
  <input id="post_img" required className="post__modal__input" type="text" placeholder="Random Image" />
  <input id="post_hash" required className="post__modal__input" type="text" placeholder="Hashtags" />
  <button type="submit" onClick={()=>handlePost()} className="btn">Create Post</button>
</div>
</>
:<></>}
      </div>
